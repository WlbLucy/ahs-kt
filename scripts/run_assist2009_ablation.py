...
EXPERIMENTS = [
    {
        "name": "target_only",
        "config_path": PROJECT_ROOT / "configs" / "ahskt_assist2009_ablation_target_only.json",
    },
    {
        "name": "target_difficulty",
        "config_path": PROJECT_ROOT / "configs" / "ahskt_assist2009_ablation_target_difficulty.json",
    },
    {
        "name": "target_difficulty_behavior_cluster",
        "config_path": PROJECT_ROOT / "configs" / "ahskt_assist2009_v2.json",
    },
]
...
def run_experiment(experiment, seed, generated_config_dir, run_root_base, reuse_existing=False):
    ...
    if reuse_existing and metrics_path.exists():
        return config, load_json(metrics_path), metrics_path
    subprocess.run(
...
def main():
    parser = argparse.ArgumentParser(description="Run ASSIST2009 ablation experiments with multiple seeds.")
    parser.add_argument("--seeds", default="2024,2025,2026")
    parser.add_argument("--output-prefix", default="assist2009_ablation_seed3")
